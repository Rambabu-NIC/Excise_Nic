...
{
    public partial class DiffPaymentDtls : System.Web.UI.Page
    {
        ...
        protected void Getdata()
        {
            try
            {
                ...
                //if (dt1.Rows.Count > 0)
                //{
                //    if (dt1.Rows[0]["BankStatus"].ToString().ToUpper() == ("Success").ToUpper())
                //    {
                //        btn_Save.Visible = true;
                //        divPayment.Visible = false;
                //        t1.Visible = true;
                //        lblReGNoP.Text = Session["RegId"].ToString();
                //        lblAmt.Text = Session["DiffAmt"].ToString(); //dt1.Rows[0]["License_Fee"].ToString();
                //        lblApplicantName.Text = dt1.Rows[0]["App_Name"].ToString();
                //        lblchno.Text = dt1.Rows[0]["ChallanNumber"].ToString();
                //        lblbankdate.Text = dt1.Rows[0]["BankDate"].ToString();
                //        lblbankStatus.Text = dt1.Rows[0]["BankStatus"].ToString();
                //    }
                //}
                //else
                //{
                ...
                string myval = objCommon.ShaEncrypt(ViewState["KeyGenerator"].ToString());
                string value = objCommon.ShaEncrypt(Session["DiffAmt"].ToString() + myval.ToLower());
                Session["DiffAmtLicenseFeeHash"] = value;
                lblFee.Text = Session["DiffAmt"].ToString();
        ...
        protected void btn_Save_Click(object sender, EventArgs e)
        {
            ...
            {
                string myval = objCommon.ShaEncrypt(ViewState["KeyGenerator"].ToString());
                string value = objCommon.ShaEncrypt(lblFee.Text + myval.ToLower());

                if (value.ToString() != Session["DiffAmtLicenseFeeHash"].ToString())
                {
                ...
                string pamount = Convert.ToDecimal(lblFee.Text.Trim()).ToString();
                objBE.IpAddress = Request.ServerVariables["REMOTE_ADDR"].ToString();
                ...
                sb.AppendFormat("<input type='hidden' name='amount' value='{0}'>", pamount);//
                //sb.AppendFormat("<input type='hidden' name='dru' value='{0}'>", "http://localhost/Excise/Event/PaymentView");
                sb.AppendFormat("<input type='hidden' name='dru' value='{0}'>", WebApiConfig.paymentredirect + "Event/PaymentView");
